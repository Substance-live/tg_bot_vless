...
from services.subscription import get_user_by_id, list_user_profiles
...
async def _render_card(cb: CallbackQuery, target: User) -> None:
    text = msg.user_card_text(target.name, target.telegram_id, target.is_active)
